...
const handleMessage = async (sock, msg) => {
  try {
    ...
    // Antisticker Logic
    try {
      const antisticker = require('./commands/admin/antisticker');
      if (msg.message?.stickerMessage) {
        const extra = { from, reply: async (text) => await sock.sendMessage(from, { text }) };
        await antisticker.checkSticker(sock, msg, extra);
      }
    } catch (e) {
      console.error('[Antisticker Error]', e.message);
    }
      // Antispam Logic
    try {
      const antispam = require('./commands/admin/antispam');
      ...
      if (content) {
        const extra = { from, reply: async (text) => await sock.sendMessage(from, { text }) };
        await antispam.checkSpam(sock, msg, extra);
      }
    } catch (e) {
      ...
      // Antibadword Logic
    try {
      const antibadword = require('./commands/admin/antibadword');
      const extra = { from, reply: async (text) => await sock.sendMessage(from, { text }) };
      await antibadword.checkBadword(sock, msg, extra);
    } catch (e) {
    ...
    for (let participant of participants) {
      ...
      if (action === 'add' && groupSettings.welcome) {
        try {
          let profilePic;
          try {
            profilePic = await sock.profilePictureUrl(participantJid, 'image');
          } catch {
            profilePic = "https://i.postimg.cc/DyqKStwP/IMG-20260410-WA0057.png"; 
          }

          const welcomeMsg = `🎉✨ ʜᴇʏ @${displayName}, \n\n` +
                             `ᴡᴇʟᴄᴏᴍᴇ ᴛᴏ ⚖️ ${groupName} \n\n` +
                             `🚀 ʏᴏᴜ ᴊᴜsᴛ ʟᴀɴᴅᴇᴅ ɪɴ ᴀɴ \n` +
                             `ᴀᴡᴇsᴏᴍᴇ ɢʀᴏᴜᴘ!\n` +
                             `👥 ᴛᴏᴛᴀʟ ᴍᴇᴍʙᴇʀs: ${groupMetadata.participants.length}\n` +
                             `📢 ʀᴜʟᴇs: ʙᴇ ʀᴇsᴘᴇᴄᴛꜰᴜʟ & sᴛᴀʏ ᴀᴄᴛɪᴠᴇ\n\n` +
                             `       ╰┈➤ ᴘᴏᴡᴇʀᴇᴅ ʙʏ ᴛᴏᴍ ᴘʀɪᴍᴇ x`;
      ...
      // --- GOODBYE/LEFT LOGIC (Attitude & Fun) ---
      else if (action === 'remove' && groupSettings.goodbye) {
        const leftMessages = [
          `😏 ᴀʀᴇʜ @${displayName}, ᴇᴛᴏ ᴛᴀʀᴀᴛᴀʀɪ ᴠᴏɪ ᴘᴀɪʏᴀ ɢᴇʟɪ? ᴛᴏᴍ ᴇʀ ᴇʟᴀᴋᴀʏ ᴛɪᴋᴛᴇ ᴋᴏʟᴇᴊᴀ ʟᴀɢᴇ ʙᴀʙᴜ! 🔥`,
          `🥱 ᴊᴀ ʙᴀʙᴜ ᴊᴀ, ᴛᴏʀ ᴍᴏᴛᴏ ɴᴏᴏʙ ᴅᴇʀ ᴅɪʏᴇ ᴛᴏᴍ ᴘʀɪᴍᴇ x ɢʀᴏᴜᴘ ᴄʜᴏʟᴇ ɴᴀ! ɢᴏᴏᴅ ʟᴜᴄᴋ. 🐍`,
          `💀 ᴇᴋᴛᴀ ᴡᴀsᴛᴇ ᴍᴇᴍʙᴇʀ ᴋᴏᴍʟᴏ! ɴɪᴊᴇᴋᴇ ᴋɪɴɢ ᴠᴀʙᴛɪ? ᴍᴏɴᴇ ʀᴀᴋʜɪs, ʀᴀᴊᴀ ᴇᴋʜᴏɴᴏ ᴛᴏᴍ ᴇ ᴀsᴇ. 👑`,
          `👋 ʙʏᴇ ʙʏᴇ @${displayName}! ᴛᴏʀ ᴍᴏᴛᴏ ᴇᴛᴏ ᴇᴛᴛɪᴛᴜᴅᴇ ᴅɪʏᴇ ᴇɪ ɢʀᴏᴜᴘ ᴇ ᴛʜᴀᴋᴀ sᴏᴍʙʜᴏᴠ ɴᴀ. ʜᴀʜᴀ! 😂`
        ];
        
        // Randomly selects one message from the list
        const goodbyeMsg = leftMessages[Math.floor(Math.random() * leftMessages.length)] + 
                           ` \n\n       ╰┈➤ ʀᴇɢᴀʀᴅs, ᴛᴏᴍ ᴘʀɪᴍᴇ x`;
